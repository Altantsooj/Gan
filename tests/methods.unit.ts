...
describe('methods', () => {
	it('can create a new method', () => {
		const nextState = methods(initialState, new_method('Fridrich'));
		expect(JSON.stringify(nextState.methodToStageMap['Fridrich'])).to.equal('{}');
	});

	it('can create a stage map and wipe it out', () => {
		let nextState = methods(initialState, new_method('Fridrich'));
		expect(JSON.stringify(nextState.methodToStageMap['Fridrich'])).to.equal('{}');
		...
		nextState = methods(nextState, new_method('Fridrich'));
		expect(JSON.stringify(nextState.methodToStageMap['Fridrich'])).to.equal('{}');
	});
	...
	it('expects delete method to eradicate it', () => {
		let nextState = methods(initialState, new_method('Fridrich'));
		expect(JSON.stringify(nextState.methodToStageMap['Fridrich'])).to.equal('{}');
		nextState = methods(nextState, new_method('CFOP'));
