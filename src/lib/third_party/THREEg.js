...
(function (global, factory) {
	...
	*/
		...
		g.lidAngleDefault =
			p.lidAngle0 === undefined &&
			p.lidAngle1 === undefined &&
			p.lidAngle2 === undefined &&
			p.lidAngle3 === undefined &&
			p.lidAngle4 === undefined &&
			p.lidAngle5 === undefined
				? true
				: false;
		...
		g.widthP = function (t) {
			return g.width(t) - 2 * g.radius(t);
		}; // width plane
		g.heightP = function (t) {
			return g.height(t) - 2 * g.radius(t);
		}; // height plane
		g.depthP = function (t) {
			return g.depth(t) - 2 * g.radius(t);
		}; // depth plane
	...
	function buildMagicBox() {
		...
		g.setAttribute(
			'position',
			new THREE.BufferAttribute(g.positions, 3).setUsage(THREE.DynamicDrawUsage)
		);
		g.setAttribute(
			'normal',
			new THREE.BufferAttribute(g.normals, 3).setUsage(THREE.DynamicDrawUsage)
		);
	...
	function morphVerticesMagicBox(time) {
		...
		function sinuslike(x) {
			...
			if (g.contourmode === 'rounding') {
				...
				var k1 =
					g.contourmode === 'profile' ? g.profile : g.contourmode === 'bezier' ? bezier : linear;
	...
	*/
		...
		function morphPartSphere(part) {
			...
			function normalsPartSphere(part) {
				...
				for (
					var v = g.vCornersIdx[part];
					v < g.vCornersIdx[part] + ((sm + 1) * (sm + 2)) / 2;
					v++
				) {
					normal.x = 0;
					normal.y = 0;
		...
		if (!g.explodeDefault && g.explodemode === 'normal') {
			for (var f = 0; f < g.faceCount; f++) {
				...
				for (var p = 0; p < 3; p++) {
					...
					g.positions[fp + 1] =
						g.positions[fp + 1] + g.faceNormals[f3 + 1] * g.radius(t) * g.explode(t);
					g.positions[fp + 2] =
						g.positions[fp + 2] + g.faceNormals[f3 + 2] * g.radius(t) * g.explode(t);
				}
			}
		}
	}
